#!/usr/bin/env python3
"""
COMBINED INSTANCE - TWO SEPARATE SYSTEMS
Both codes run in same instance but are completely isolated
No interaction, shared memory, or dependencies
"""
...
# Different ports for each system
SITE1_PORT = 10000
SITE2_PORT = 10001

# Different thread names for clarity
SITE1_THREAD_NAME = "Site1_TripleAlert_System"
SITE2_THREAD_NAME = "Site2_UltraFastArbitrage_Bot"

# ==================== SITE 1 CODE - COMPLETE AND ISOLATED ====================
def run_site1():
    """RUN SITE 1 - Triple Alert System (unchanged)"""
    
    # Create isolated namespace for site1
    import sys
    import os
    ...
    try:
        print("="*80)
        print("🚀 STARTING SITE 1: Triple Alert System")
        print("="*80)
        
        # ════════════════════════════════════════════════════════════════════
        # ⬇️ ⬇️ ⬇️ SITE 1 CODE - PASTED EXACTLY ⬇️ ⬇️ ⬇️
        # ════════════════════════════════════════════════════════════════════
        ...
        **Alert:** Spread is {spread_percent:.1f}% (Bid: ${bid_price:.2f}, Ask: ${ask_price:.2f})
        ...
        # ════════════════════════════════════════════════════════════════════
        # ⬆️ ⬆️ ⬆️ END OF SITE 1 CODE ⬆️ ⬆️ ⬆️
        # ════════════════════════════════════════════════════════════════════
        
        print("✅ SITE 1: Triple Alert System started successfully")
        
    finally:
        # Clean up to prevent contamination with Site 2
...
# ==================== SITE 2 CODE - COMPLETE AND ISOLATED ====================
def run_site2():
    """RUN SITE 2 - Ultra Fast Arbitrage Bot (unchanged)"""
    
    # Create isolated namespace for site2
    import sys
    import os
    ...
    try:
        print("\n" + "="*80)
        print("🚀 STARTING SITE 2: Ultra Fast Arbitrage Bot")
        print("="*80)
        
        # ════════════════════════════════════════════════════════════════════
        # ⬇️ ⬇️ ⬇️ SITE 2 CODE - PASTED EXACTLY ⬇️ ⬇️ ⬇️
        # ════════════════════════════════════════════════════════════════════
        ...
        # ════════════════════════════════════════════════════════════════════
        # ⬆️ ⬆️ ⬆️ END OF SITE 2 CODE ⬆️ ⬆️ ⬆️
        # ════════════════════════════════════════════════════════════════════
        
        print("✅ SITE 2: Ultra Fast Arbitrage Bot started successfully")
        
    finally:
        # Clean up to prevent contamination with Site 1
        new_modules = set(sys.modules.keys()) - original_modules
        for module in new_modules:
            if module in sys.modules:
                del sys.modules[module]
...
# ==================== MAIN EXECUTION ====================
def main():
    """Run both sites completely separately in parallel"""
    print("\n" + "="*80)
    print("🚀 COMBINED INSTANCE - TWO SEPARATE SYSTEMS")
    print("="*80)
    print("✅ Both systems will run in the same instance")
    print("🔒 Each system is completely isolated")
    print("🌐 Different ports for each system:")
    print(f"   • Site 1 (Triple Alert): http://localhost:{SITE1_PORT}")
    print(f"   • Site 2 (UltraFast Arbitrage): http://localhost:{SITE2_PORT}")
    print("⚡ Running both in parallel threads")
    print("="*80)
    
    # Start Site 1 in separate thread
    site1_thread = threading.Thread(
        target=run_site1,
        name=SITE1_THREAD_NAME,
        daemon=True
    )
    
    # Start Site 2 in separate thread
    site2_thread = threading.Thread(
        target=run_site2,
        name=SITE2_THREAD_NAME,
        daemon=True
    )
    ...
    try:
        while True:
            ...
            if int(time.time()) % 30 == 0:
                print(f"💓 Combined instance running... Site 1 & Site 2 active")
    except KeyboardInterrupt:
        print("\n🛑 Shutting down combined instance...")
